...
function addButtonEvent() {
    addButton.forEach(animal => animal.addEventListener("click", setStorage));
}
addButtonEvent();

// Fonction de l'event du boutton 'Vider toute la liste' des annonces de la page des favoris
function clearButtonEvent() {
    clearButton.forEach(animal =>
        animal.addEventListener("click", clearStorage)
    );
}
clearButtonEvent();
...
function removeButtonEvent() {
    removeButton.forEach(animal =>
        animal.addEventListener("click", removeItemFromStorage)
    );
}
removeButtonEvent();
